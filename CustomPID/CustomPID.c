...
int maxSat = 125;
//max saturation for motor thottle, range between 0-127
int closeEnough = 10;
//degrees off of set point before motor controller activates
int minSat = 20;
//minimum saturation, motors are weak and friction of ball bearing prevented
...
//roboclaw uses direction functions,
//therefore we required a boolean for negative error
bool isForward = true;
...
void setup(){
  ...
  Setpoint_x = event.orientation.x;
  //sets the 0 of setpoint to first read value of IMU
  Setpoint_y = 0;
  Setpoint_z = 0;
}

// Functions to calculate attitude error.
//Intput is the setpoint angle and a attitude between 0-360 degrees
// Returns values adjusted so maximum absolute error is 1 pi
...
// Integral and Derivate PID controler lines commented out
// for ease of P gain implementation
...
void loop(){
  ...
  Serial.print("Z: "); //the z-axis of our satellite actually uses
  //the x attitude sensor because the IMU is mounted on the side
...
//  Serial.print(event.orientation.z, 4);
  ...
  //below code utlizes the gryo as a method
  //to ensure if acceleration is 0 even at full speed
